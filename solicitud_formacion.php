...
<head>
    ...
    <title>Nueva Solicitud · Unicauca</title>
...
<div id="contenido">
    <div class="uc-page-wrapper">
        <div class="uc-card-header">
            <h5 class="uc-card-title"><i class="fas fa-pen-alt"></i> Solicitud de Comisión</h5>
        ...
        <div class="p-3">
            <form action="nueva_solicitud_formacion.php" method="post">
                ...
                <!-- Fila 2: Resolución y tipo -->
                <div class="form-row">
                    <div class="form-group col-md-3">
                        <label>No. Resolución *</label>
                        <input type="text" class="form-control" name="No_resolucion" required>
                    </div>
                    <div class="form-group col-md-3">
                ...
                <div id="destinos">
                    <div class="form-row destino mb-2">
                        ...
                        <div class="col-md-2 d-flex align-items-end">
                            <button type="button" class="add-destination-btn" onclick="agregarDestino()"><i class="fas fa-plus"></i> Agregar</button>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-row">
                    ...
                    <div class="form-group col-md-3">
                        ...
                        <input type="date" class="form-control" name="fecha_aval">
                    </div>
                    <div class="form-group col-md-3">
                        <label>Duración Horas</label>
                        <input type="number" class="form-control" id="duracion_horas" name="duracion_horas" readonly>
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group col-md-3">
                        <label>Evento *</label>
                        <textarea class="form-control" name="evento" id="evento" rows="2" oninput="detectarModalidad()" required></textarea>
                    </div>
                    <div class="form-group col-md-3">
                        <label>Organizado Por</label>
                        <textarea class="form-control" name="organizado_por" rows="2"></textarea>
                    </div>
                    <div class="form-group col-md-3">
                        <label>Nombre del Trabajo</label>
                        <textarea class="form-control" name="nombre_trabajo" rows="2"></textarea>
                    </div>
                    <div class="form-group col-md-3">
                ...
                <!-- Presupuesto (bloque compacto) -->
                <div class="bordered-section">
                    <div class="section-label">Presupuesto</div>
                    <div class="form-row">
                        <div class="form-group col-md-4">
                            <div class="custom-control custom-checkbox">
                                <input type="checkbox" class="custom-control-input" id="viaticos" name="viaticos" value="1" onclick="handleViaticosChange()">
                                <label class="custom-control-label" for="viaticos">Viáticos</label>
                            </div>
                        </div>
                        <div class="form-group col-md-4">
                            <div class="custom-control custom-checkbox">
                                <input type="checkbox" class="custom-control-input" id="tiquetes" name="tiquetes" value="1" onclick="handleViaticosChange()">
                                <label class="custom-control-label" for="tiquetes">Tiquetes</label>
                            </div>
                        </div>
                        <div class="form-group col-md-4">
                            <div class="custom-control custom-checkbox">
                                <input type="checkbox" class="custom-control-input" id="inscripcion" name="inscripcion" value="1" onclick="handleViaticosChange()">
                    ...
                    <div id="cargoFields" style="display: none;">
                        <div class="form-group">
                            <label>Cargo A</label>
                            <input type="text" class="form-control" name="cargo_a">
                        </div>
                    </div>
                ...
                <div class="form-row">
                    <div class="form-group col-md-3">
                        <label>Rector *</label>
                        <select class="form-control" name="id_rector" required>
                            <?php while ($rector = $rectores_result->fetch_assoc()): ?>
                                <option value="<?= $rector['rector_cc'] ?>"><?= htmlspecialchars($rector['rector_nombre']) ?></option>
                            <?php endwhile; ?>
                        </select>
                    </div>
                    <div class="form-group col-md-3">
                        <label>Vicerrector *</label>
                        <select class="form-control" name="id_vice" required>
                            <?php while ($vicerrector = $vicerrectores_result->fetch_assoc()): ?>
                                <option value="<?= $vicerrector['vice_cc'] ?>"><?= htmlspecialchars($vicerrector['vice_nombre']) ?></option>
                            <?php endwhile; ?>
                        </select>
                    </div>
                    <div class="form-group col-md-3">
                        ...
                        <select class="form-control" name="tramito" required>
                            ...
                            ?>
                                <option value="<?= htmlspecialchars($user['name']) ?>" <?= $selected ?>><?= htmlspecialchars($user['name']) ?></option>
                            <?php endwhile; ?>
                        </select>
                    </div>
                    <div class="form-group col-md-3">
                        ...
                        <select class="form-control" name="reviso" required>
                            <?php while ($revisau = $revisa_result->fetch_assoc()): ?>
                                <option value="<?= htmlspecialchars($revisau['revisa_nom_propio']) ?>"><?= htmlspecialchars($revisau['revisa_nom_propio']) ?></option>
                            <?php endwhile; ?>
                        </select>
                    </div>
                </div>
                ...
                <div class="form-row mt-3">
                    <div class="col-md-6">
                        <button type="submit" class="btn btn-uc-primary btn-block"><i class="fas fa-save"></i> Guardar Solicitud</button>
                    </div>
                    <div class="col-md-6">
...
<script>
// Listas de autocompletado (paises y ciudades)
...
function calculateHours() {
    var start = new Date(document.getElementById("fecha_inicio").value);
    var end = new Date(document.getElementById("fecha_fin").value);
    if (start && end && !isNaN(start) && !isNaN(end) && end >= start) {
        var days = (end - start) / (1000 * 60 * 60 * 24) + 1;
        document.getElementById("duracion_horas").value = days * 8;
    }
}
...
function detectarModalidad() {
    ...
    const esOnline = terminosOnline.some(termino => evento.includes(termino));
    modalidad.value = esOnline ? "Online" : "Presencial";
}

function handleViaticosChange() {
    const viaticos = document.getElementById("viaticos").checked;
    const tiquetes = document.getElementById("tiquetes").checked;
    const inscripcion = document.getElementById("inscripcion").checked;
    const cargoAdmin = "<?= addslashes($profesor['CARGO_ADMIN'] ?? '') ?>";
    const necesitaCampos = (viaticos || tiquetes || inscripcion);
    document.getElementById("cargoFields").style.display = necesitaCampos ? "block" : "none";
    if (necesitaCampos && ["JEFE", "DECANO", "DIRECTOR"].includes(cargoAdmin.toUpperCase())) {
...
$(document).ready(function() {
    applyAutocomplete();
    $('#fecha_inicio, #fecha_fin').on('blur', validateDates);
    $('#evento').on('input', detectarModalidad);
    $('#tipo_estudio').on('change', handleTipoEstudioChange);
    $('.form-check-input').on('change', handleViaticosChange);
