...
if ($tipo_vice == 'propiedad') {
    ...
    if ($viaticos == 1 || $tiquetes == 1 || $inscripcion == 1) {
        $texto = ' genera erogaciones a la Universidad del Cauca, por concepto de ';
    ...
    $section->addText('RESOLUCIÓN VRA N°' . $res.' DE  ' . $anio, array('bold' => true, 'size' => 10), array('alignment' => Jc::CENTER,'spaceAfter' => 0));
 // Añadir el título centrado
    $section->addText('(' . $fecha_dia_mes.')', array('bold' => true, 'size' => 10), array('alignment' => Jc::CENTER));


    // Añadir el primer párrafo
    $paragraphStyle = array('alignment' => Jc::BOTH, 'spaceAfter' => 150);
    $textRun = $section->addTextRun($paragraphStyle);
    $textRun->addText('Por la cual se autoriza una comisión académica en el territorio nacional ', null);
...
$textRun->addText('.');
    ...
    if ($tipo_vice == 'propiedad') {
        ...
        $textRun->addText(', en uso de competencias establecidas en el Acuerdo Superior 024 de 1993 - Estatuto del Profesor de la Universidad del Cauca, principalmente conforme a lo previsto en el artículo 73, modificado por el artículo quinto del Acuerdo Superior 031 de 2020, y conforme a la ' . $res_encargo_vice . ', y');
    } elseif ($$tipo_vice == 'delegatario') {
        $textRun->addText('EL RECTOR DELEGATARIO DE LA UNIVERSIDAD DEL CAUCA', array('bold' => true));
        $textRun->addText(', en uso de sus competencias previstas mediante el Acuerdo 105 de 1993, y conforme a la ' . $res_encargo_vice . ', y');
    }
     // Añadir el título centrado
...
// Añadir el título centrado
$section->addText('CONSIDERANDO QUE:', array('bold' => true, 'size' => 10), array('alignment' => Jc::CENTER, 'spaceAfter' => 150));
...
$textRun->addText('.', null);
  ...
  $textRun = $section->addTextRun($paragraphStyle);
     $textRun->addText('Mediante la Resolución Rectoral 0901 de 2023, se reglamentaron los requisitos para la autorización y concesión de las comisiones académicas al interior y al exterior del país de los profesores.');  
...
$textRun->addText('El Consejo de la ' . $facultad_min . ', en sesión del día ' . $fecha_letras_aval . ', avaló la Comisión Académica en el territorio nacional ' . $saludode . ' ', null);
$textRun->addText($nombre_profesor, array('bold' => true));
$textRun->addText(', ', null);
...
$textRun->addText($nombre_profesor, array('bold' => true));
$textRun->addText(', al evento ', null);

$textRun->addText($evento, array('italic' => true));

$textRun->addText(', organizado por ' . $organizado_por . ', ' . $endestinos . $destinos .  $parrafoini_trabajo. '.', null);
...
// Añadir el título centrado
$section->addText('RESUELVE:', array('bold' => true, 'size' => 10), array('alignment' => Jc::CENTER,'spaceAfter' => 150));   
    ...
    $textRun->addText('ARTÍCULO PRIMERO: ', array('bold' => true));
     ...
     $textRun->addText('Autorizar comisión académica en el territorio nacional, '. obtenerFechasFormateadas($fechainicio1, $fechafin1) . ', ',null);
...
$textRun->addText($documento_tercero, array('bold' => true));
$textRun->addText(', ' . $adscrito . ' al departamento de ' . $depto_nom_propio, null); 
 $textRun->addText(', con el fin de participar en el evento ' . $evento . ', organizado por ' . $organizado_por . ', ' . $endestinos . $destinos . '.', null);
$paragraphStyle = array('alignment' => Jc::BOTH, 'spaceAfter' => 150);
    $textRun = $section->addTextRun($paragraphStyle);
...
if ($cargo_admin == "DECANO") {
    ...
    $textRun->addText('Como deber especial, ' . $comisionado . ' rendirá a la Decanatura de su Facultad con copia en físico a esta Vicerrectoría, un informe escrito dentro de los diez (10) días siguientes al vencimiento de la comisión. En el informe se presentarán los aspectos pertinentes a los objetivos específicos de la unidad académica y generales de la Institución, de lo cual, se dejará constancia en el Acta de Reunión de Departamento.', null);
}
...
// Añadir el título centrado
    $section->addText($nombre_vice, array('bold' => true, 'size' => 10), array('alignment' => Jc::CENTER, 'spaceAfter' => 0));   

    $section->addText($cargov, array('bold' => false, 'size' => 10), array('alignment' => Jc::CENTER));   
