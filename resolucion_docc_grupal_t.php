...
// Añadir el título centrado
$section->addText('CONSIDERANDO QUE:', array('bold' => true, 'size' => 12), array('alignment' => Jc::CENTER, 'spaceAfter' => 540));
...
$textRun->addText('En sesión del ' . $fecha_letras_aval . ', el Consejo de la ' . $facultad_min . ' aprobó la participación de unas comisiones académica en el evento "' . $evento . '", organizado por ' . $organizado_por . ', que se llevará a cabo ' . $endestinos . $destinos . ', y de acuerdo con los documentos que respaldan la solicitud, se considera pertinente para la Universidad la participación de los mencionados profesores, como se detalla a continuación:', array('size' => 12));

// Inicializamos arrays para almacenar los profesores agrupados por departamento y los géneros de los profesores en cada departamento
// Definimos el estilo de la tabla
$trabajo_existe = false;

// Primer ciclo: verificar si algún profesor tiene trabajo asignado
// Inicializar un array para almacenar los datos de los profesores
$datos_profesores = [];

// Primer ciclo: recoger los datos
while ($row = $resultadores->fetch_assoc()) {
    if (!empty($row['nombre_trabajo'])) {
        $trabajo_existe = true;
        break; // Si encontramos uno, podemos dejar de buscar
    }
}

// Resetear el puntero del resultado para iterar nuevamente
$resultadores->data_seek(0);
...
$phpWord->addTableStyle('SimpleTable', $styleTable);
$table = $section->addTable('SimpleTable');
$table->setWidth('100%');
...
// Añadir encabezados según si hay trabajos o no
$row = $table->addRow();
$row->addCell(1200, $headerCellStyle)->addText('Documento', $cellTextStyle);
$row->addCell(2000, $headerCellStyle)->addText('Nombre', $cellTextStyle);
$row->addCell(1500, $headerCellStyle)->addText('Departamento', $cellTextStyle);
if ($trabajo_existe) {
    $row->addCell(3000, $headerCellStyle)->addText('Trabajo', $cellTextStyle); // Columna "Trabajo" si existe
}
$row->addCell(1500, $headerCellStyle)->addText('Fechas', $cellTextStyle); // Columna "Fechas"

// Segundo ciclo: iterar sobre los resultados y almacenar los datos
while ($row = $resultadores->fetch_assoc()) {
    $documento_tercero = formatDocumentNumber($row['documento_tercero']);
    $nombre1 = $row['nombre1'];
    $nombre2 = $row['nombre2'];
    ...
    // Extraer el trabajo si existe
    $trabajo = isset($row['nombre_trabajo']) ? $row['nombre_trabajo'] : '';
    ...
    $datos_profesores[] = [
        ...
        'nombre' => utf8_decode($nombre_profesor),
        'departamento' => utf8_decode($departamento),
        'trabajo' => $trabajo,
        'fechas' => utf8_decode($fechas_formateadas)
    ];
}

// Ordenar el array por departamento
usort($datos_profesores, function($a, $b) {
    return strcmp($a['departamento'], $b['departamento']);
});

// Rellenar la tabla con los datos ordenados
foreach ($datos_profesores as $data) {
    $row = $table->addRow();
    $row->addCell(1200, array('borderSize' => 1, 'cellMargin' => 0))->addText($data['documento'], $cellTextStyle);
    $row->addCell(2000, array('borderSize' => 1, 'cellMargin' => 0))->addText($data['nombre'], $cellTextStyle);
    $row->addCell(1500, array('borderSize' => 1, 'cellMargin' => 0))->addText($data['departamento'], $cellTextStyle);
    
    // Solo agregar la columna de trabajo si hay trabajos
    if ($trabajo_existe) {
        $row->addCell(3000, array('borderSize' => 1, 'cellMargin' => 0))->addText($data['trabajo'], $cellTextStyle);
    }
    
    // Añadir las fechas
    $row->addCell(1500, array('borderSize' => 1, 'cellMargin' => 0))->addText($data['fechas'], $cellTextStyle);
}
...
// Añadir el texto inicial solo una vez
// Añadir la primera parte del texto, incluyendo el evento
 
// Añadir el texto final para todos los profesores solo una vez
$textRun->addText('Siendo este un evento de gran proyección nacional, que le permitirá compartir sus conocimientos y adquirir nuevos, compartir su experiencia académica y profesional, fortalecer lazos interinstitucionales y dar visibilidad a su Programa y al Alma Máter.', array('size' => 12));
...
$section->addText('RESUELVE:', array('bold' => true, 'size' => 12), array('alignment' => Jc::CENTER));   
    ...
    $paragraphStyle = array('alignment' => Jc::BOTH, 'spaceAfter' => 250);
    $textRun = $section->addTextRun($paragraphStyle);
...
// Añadir el texto introductorio para la comisión académica
$textRun->addText('Autorizar comisión académica en el territorio nacional ', array('size' => 12));

$textRun->addText(' con el fin de participar en el evento "', array('size' => 12));
...
$textRun->addText('", organizado por ' . $organizado_por . ', en ' . $destinos . ' a los siguientes profesores: ', array('size' => 12));
$textRun->addText(' de la Facultad de ' . $facultad_min . ':', array('size' => 12));
...
$row = $table->addRow();
$row->addCell(1500, $headerCellStyle)->addText('Cédula', $cellTextStyle);
$row->addCell(3000, $headerCellStyle)->addText('Nombre', $cellTextStyle);
$row->addCell(2000, $headerCellStyle)->addText('Departamento', $cellTextStyle);
$row->addCell(2000, $headerCellStyle)->addText('Fechas', $cellTextStyle);

// Inicializar un array para almacenar los datos
$datos_profesores = [];

// Inicializar un array para almacenar los departamentos
$departamentos = []; // Asegúrate de que esta variable esté definida

// Ciclo para recopilar los datos de los profesores
$resultadores->data_seek(0);  // Reiniciar el puntero de resultados
while ($row = $resultadores->fetch_assoc()) {
    $nombre1 = $row['nombre1'];
    $nombre2 = $row['nombre2'];
    $apellido1 = $row['apellido1'];
    ...
    $datos_profesores[] = [
        'documento' => $documento_tercero,
        'nombre' => utf8_decode($nombre_profesor),
        'departamento' => utf8_decode($row['depto_nom_propio']),
        'fechas' => utf8_decode($fechas_formateadas),
    ];

    // Añadir departamento al array si no está ya presente
    if (!in_array($row['depto_nom_propio'], $departamentos)) {
        $departamentos[] = $row['depto_nom_propio'];
    }
}

// Ordenar el array por el departamento
usort($datos_profesores, function($a, $b) {
    return strcmp($a['departamento'], $b['departamento']);
});
...
foreach ($datos_profesores as $data) {
    $fila = $table->addRow();
    $fila->addCell(1500, array('borderSize' => 1, 'cellMargin' => 0))->addText($data['documento'], $cellTextStyle);
    $fila->addCell(3000, array('borderSize' => 1, 'cellMargin' => 0))->addText($data['nombre'], $cellTextStyle);
    $fila->addCell(2000, array('borderSize' => 1, 'cellMargin' => 0))->addText($data['departamento'], $cellTextStyle);
    $fila->addCell(2000, array('borderSize' => 1, 'cellMargin' => 0))->addText($data['fechas'], $cellTextStyle);
}
...
if (count($departamentos) > 1) {
    ...
    $departamentos_texto = 'adscrito al Departamento de ' . $departamentos[0];
}
...
$paragraphStyle = array('alignment' => Jc::BOTH, 'spaceAfter' => 150);
 ...
 // Inicializar acumuladores de erogaciones
$viaticos_acum = false;
$tiquetes_acum = false;
$inscripcion_acum = false;

// Inicializar array para acumular los cargos
$cargos_acum = [];

// Volver al inicio de los resultados
$resultadores->data_seek(0);

// Ciclo para procesar los profesores
while ($row = $resultadores->fetch_assoc()) {
    $viaticos = (int)$row['viaticos'];  // Convertir a entero por seguridad
    $tiquetes = (int)$row['tiquetes'];  // Convertir a entero
    $inscripcion = (int)$row['inscripcion'];  // Convertir a entero
    $cargo_a = $row['cargo_a'];

    // Verificar si algún profesor genera viáticos, tiquetes o inscripción
    if ($viaticos === 1) $viaticos_acum = true;
    if ($tiquetes === 1) $tiquetes_acum = true;
    if ($inscripcion === 1) $inscripcion_acum = true;

    // Acumular cargos si hay y si no está ya en el array
    if (!empty($cargo_a) && !in_array($cargo_a, $cargos_acum)) {
        $cargos_acum[] = $cargo_a;
    }
}

// Generar el texto de erogaciones
$erogaciones = 'no genera erogaciones a la Universidad del Cauca, en viáticos, inscripción o tiquetes.';

// Si al menos uno de los conceptos (viáticos, tiquetes, inscripción) tiene valor 1
$items = [];
if ($viaticos_acum) $items[] = 'viáticos';
if ($tiquetes_acum) $items[] = 'tiquetes';
if ($inscripcion_acum) $items[] = 'inscripción';

if (count($items) > 0) {
    $erogaciones = 'genera erogaciones a la Universidad del Cauca por concepto de ';

    // Formatear la lista de conceptos
    if (count($items) > 1) {
        $last_item = array_pop($items);  // Extraer el último item
        $erogaciones .= implode(', ', $items) . ' y ' . $last_item;
    } else {
        $erogaciones .= $items[0];  // Solo un item
    }
}

// Generar el texto de cargos
$cargos_texto = '';
if (count($cargos_acum) > 0) {
    $cargos_texto = ' con cargo al presupuesto de ';

    // Formatear la lista de cargos
    if (count($cargos_acum) > 1) {
        $last_cargo = array_pop($cargos_acum);  // Extraer el último cargo
        $cargos_texto .= implode(', ', $cargos_acum) . ' y ' . $last_cargo;
    } else {
        $cargos_texto .= $cargos_acum[0];
    }
}

// Texto final de la comisión
$texto_erogaciones = 'La comisión autorizada ' . $erogaciones . $cargos_texto . '.';

// Añadir el texto generado al documento
$textRun->addText($texto_erogaciones, array('size' => 12));
$paragraphStyle = array('alignment' => Jc::BOTH, 'spaceAfter' => 250);
...
$filename = "Resolucion - $tipo_estudio-$destinos-$fecha_formateada.docx";
