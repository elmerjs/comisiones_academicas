    <?php
session_start();
$currentYear = date("Y");
?>

<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Gestión Comisiones Académicas Docentes Unicauca</title>

    <style>
        /* Estilos del menú lateral y el submenú */
        .submenu {
            display: none; /* Ocultar el submenú por defecto */
        }

        .submenu.visible {
            display: block; /* Mostrar el submenú cuando tiene la clase 'visible' */
        }

        /* Estilos del menú lateral */
        #menu-lateral {
            background-color: #EEEBEB; /* Azul oscuro */
            width: 200px;
            height: 100%;
            position: fixed;
            top: 0;
            left: 0;
            padding-top: 90px; /* Altura del encabezado disminuida */
            color: white;
            font-family: Arial, sans-serif;
            z-index: 98; /* Asegurar que esté detrás del encabezado */
            transition: transform 0.3s ease;
        }

        #menu-lateral ul {
            list-style: none;
            padding: 0;
            margin: 0;
        }

        #menu-lateral li {
            margin-bottom: 10px;
        }

        #menu-lateral a {
            display: block;
            padding: 10px 20px;
            text-decoration: none;
            color: black;
            transition: all 0.3s ease;
        }

        #menu-lateral a:hover {
            background-color: #6B70C4; /* Azul más claro */
        }

        /* Estilos del contenido */
        #contenido {
            margin-left: 220px; /* Aumentar el margen para dejar espacio al menú */
            padding: 20px;
            z-index: 1; /* Asegurar que esté adelante del menú */
            transition: margin-left 0.3s ease;
        }

        /* Estilos del encabezado */
        header {
            background: linear-gradient(#060264, #001f33); /* Azul */
            width: 100%;
            height: 50px; /* Altura del encabezado reducida */
            position: fixed;
            top: 0;
            left: 0;
            z-index: 99; /* Asegurar que esté adelante del menú */
            color: white;
        }

        header h1 {
            margin: 0;
            font-size: 20px; /* Tamaño de fuente reducido */
            line-height: 50px; /* Altura del encabezado */
            padding-left: 20px;
            font-weight: bold; /* Texto más impactante */
        }

        /* Estilos del login */
        #login {
            position: fixed;
            top: 0;
            right: 0;
            padding: 10px;
            color: white; /* Texto blanco */
            font-family: Arial, sans-serif;
        }

        #login a {
            color: white; /* Color del enlace blanco */
        }

        /* Estilos para el botón de mostrar/ocultar menú */
        #toggle-menu {
            background-color: #6B70C4;
            color: white;
            border: none;
            padding: 5px;
            cursor: pointer;
            width: 80px;
            text-align: left;
            font-size: 16px;
            transition: background-color 0.3s ease;
            position: fixed;
            top: 50px;
            left: 0;
            z-index: 100;
        }

        #toggle-menu:hover {
            background-color: #6B70d4;
        }

        @media screen and (max-width: 768px) {
            /* Estilos para pantallas pequeñas */
            #menu-lateral {
                width: 150px; /* Reducir el ancho del menú */
            }

            #contenido {
                margin-left: 170px; /* Aumentar el margen para dejar espacio al menú */
            }

            header {
                height: 70px; /* Altura del encabezado aumentada */
                line-height: 70px; /* Ajustar línea de altura */
            }

            header h1 {
                font-size: 18px; /* Tamaño de fuente reducido */
                line-height: 70px; /* Ajustar línea de altura */
            }

            #toggle-menu {
                width: 150px;
            }
        }

        nav ul {
            list-style: none;
            padding: 0;
            margin: 0;
        }

        nav ul li {
            position: relative;
        }

        nav ul li a {
            text-decoration: none;
            display: block;
            padding: 10px;
            color: white;
        }

        nav ul li ul.submenu {
            display: none;
            position: absolute;
            top: 0;
            left: 100%;
            list-style: none;
            padding: 0;
            margin: 0;
            background-color: #EEEBEB   ;
            border: 1px solid #ccc;
            z-index: 1000;
        }

        nav ul li ul.submenu li {
            width: 200px; /* Ajustar el ancho según sea necesario */
        }

        nav ul li ul.submenu li a {
            padding: 10px;
            color: white;
        }

        nav ul li.submenu-container:hover ul.submenu {
            display: block;
        }

        nav ul li a:hover,
        nav ul li ul.submenu li a:hover {
            background-color: #001f33; /* Azul más claro */
        }
    </style>
</head>
<body>
    <div id="menu-lateral">
        <nav>
            <ul>
                <li>
                    <a href="../../comisiones_academicas/report_terceros.php">Tramitar por Profesor</a>
                </li>
                
                <li class="submenu-container">
                    <a href="#">Comisiones</a>
                    <ul class="submenu">
                        <li><a href="../../comisiones_academicas/comisionesb.php?anio=<?php echo $currentYear; ?>"><?php echo $currentYear; ?></a></li>
                        <li><a href="../../comisiones_academicas/comisionesb.php">General</a></li>
                    </ul>
                </li>
                
                
                <li class="submenu-container">
                    <a href="#">Informes de Comisión</a>
                    <ul class="submenu">
                        <li><a href="../../comisiones_academicas/report_pendientes.php?anio=<?php echo $currentYear; ?>"><?php echo $currentYear; ?></a></li>
                        <li><a href="../../comisiones_academicas/report_pendientes.php">General</a></li>
                    </ul>
                </li>
                  <li><a href="../../comisiones_academicas/sinlegalizar.php"> Emails Inf. pendientes</a></li>
                <li><a href="../../comisiones_academicas/directivos.php">Gestionar Encargos</a></li>
                <li><a href="../../comisiones_academicas/powerbics.php">PB-Gráficos</a></li>
            </ul>
        </nav>
    </div>

    <button id="toggle-menu">☰ Menú</button>

    <header>
        <?php
        if (isset($_SESSION['loggedin'])) {
            // checking the time now when check-login.php page starts
            $now = time();           
            if ($now > $_SESSION['expire']) {
                session_destroy();
                echo "<div class='alert alert-danger mt-4' role='alert'>
                <h4>Your session has expired!</h4>
                <p><a href='/comisiones_academicas/index.html'>Login Here</a></p></div>";
                exit;
            }
        } else {
            echo "<div class='alert alert-danger mt-4' role='alert'>
                <h4 style='color: white;'>¡Necesita iniciar sesión para acceder a esta página! <a href='/comisiones_academicas/index.html' style='color: white;'>¡Inicie sesión aquí!</a></h4>
            </div>";
            exit;
        }
        ?>
        <h1>Comisiones Académicas Unicauca</h1>
        <div id="login">
            <?php
                if (isset($_SESSION['loggedin'])) {
                    echo "<i>" . $_SESSION['name'] . "</i> <a href='../../comisiones_academicas/logout.php'>Logout</a>";
                } else {
                    echo "<div class='alert alert-danger mt-4' role='alert'>
                        <h4>You need to login to access this page.</h4>
                        <p><a href='/comisiones_academicas/index.html'>Login Here!</a></p>
                    </div>";
                }
            ?>
        </div>
    </header>

        <!-- Tu contenido aquí -->


    <script>
        document.getElementById('toggle-menu').addEventListener('click', function() {
            var menu = document.getElementById('menu-lateral');
            var contenido = document.getElementById('contenido');
            
            if (menu.style.transform === 'translateX(-200px)') {
                menu.style.transform = 'translateX(0)';
                contenido.style.marginLeft = '220px';
            } else {
                menu.style.transform = 'translateX(-200px)';
                contenido.style.marginLeft = '20px';
            }
        });
    </script>
</body>
</html>
