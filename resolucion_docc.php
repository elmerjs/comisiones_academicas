...
$sectionStyle = array(
    'paperSize' => 'Folio',
    'marginTop' => 3000,  // 5 cm
'marginLeft' => 1701, // 2,5 cm
'marginRight' => 1701, // 2,5 cm
...
if ($dia == 1) {
    $dia_en_letras = "el día primero(1) ";
} else {
...
if ($tipo_vice == 'propiedad') {
    if ($sexo_vice == 'F') {
        ...
        $nombrarvicel = "la Vicerrectora Académica ";
...
$textRun->addText('.');
    ...
    // Verificar el género del vicerrector o vicerrectora
    if ($sexo_vice == 'F') {
        $textRun->addText('La ', array('size' => 12));

        $textRun->addText('VICERRECTORA ACADÉMICA ENCARGADA DE LA UNIVERSIDAD DEL CAUCA', array('bold' => true, 'size' => 12));
    } else {
...
$textRun->addText('El Consejo de la ' . $facultad_min . ', en sesión del día ' . $fecha_letras_aval . ', avaló la comisión académica en el territorio nacional ' . $saludode . ' ',array('size' => 12));
$textRun->addText($nombre_profesor, array('bold' => true,'size' => 12));
$textRun->addText(', ',array('size' => 12));
...
$textRun->addText($nombre_profesor, array('bold' => true,'size' => 12));
$textRun->addText(', al evento "',array('size' => 12));

$textRun->addText($evento, array('italic' => true,'size' => 12));

$textRun->addText('", organizado por ' . $organizado_por . ', ' . $endestinos . $destinos .  $parrafoini_trabajo. '.',array('size' => 12));
...
$section->addText('RESUELVE:', array('bold' => true, 'size' => 12), array('alignment' => Jc::CENTER));   
    ...
    $textRun->addText('ARTÍCULO PRIMERO: ', array('bold' => true,'size' => 12));
     ...
     $textRun->addText('Autorizar comisión académica en el territorio nacional, '. obtenerFechasFormateadas($fechainicio1, $fechafin1) . ', ',array('size' => 12));
...
// Agregar el texto con el evento en cursiva en la nueva línea
$textRun->addText(', con el fin de participar en el evento "', array('size' => 12));

// Agregar el evento en cursiva nuevamente
$textRun->addText($evento, array('italic' => true, 'size' => 12));
