...
if ($resultadores->num_rows > 0) {
    ...
    // si es virtual:
   if ($modalidad == 'Online') {
        $enmodalidad = " en modalidad virtual, ";
...
if ($tipo_vice == 'propiedad') {
    ...
    if ($viaticos == 1 || $tiquetes == 1 || $inscripcion == 1) {
        $texto = ' podrá generar erogaciones a la Universidad del Cauca, en gastos de ';
...
/*
    ...
    $section->addText('RESOLUCIÓN RECTORAL No.                          DE  ' . $anio, array('bold' => true, 'size' => 12), array('alignment' => Jc::CENTER,'spaceAfter' => 0));
 // Añadir el título centrado
    $section->addText('(                       )', array('bold' => true, 'size' => 12), array('alignment' => Jc::CENTER));
...
$textRun->addText('Por la cual se concede comisión académica al exterior ' . $enmodalidad . ' a un Profesor con Cargo Académico Administrativo.', array('size' => 12));

} else {
           $textRun->addText('Por la cual se concede comisión académica al exterior'. $enmodalidad . ' ',   array('size' => 12));
           $textRun->addText($saludo, array('bold' => false,  'size' => 12));    
...
// Añadir el título centrado
$section->addText('CONSIDERANDO QUE:', array('bold' => true, 'size' => 12), array('alignment' => Jc::CENTER, 'spaceAfter' => 10));
$section->addTextBreak(1);

    $paragraphStyle = array('alignment' => Jc::BOTH, 'spaceAfter' => 150);
    $textRun = $section->addTextRun($paragraphStyle);
     $textRun->addText('El Acuerdo 024 de 1993 establece en su Capítulo XIV la comisión académica, entendida como la asistencia a foros, viajes de estudio, seminarios, congresos, encuentros, cursos o similares en los cuales el profesor representa a la Universidad del Cauca.',  array('size' => 12));
 
     $paragraphStyle = array('alignment' => Jc::BOTH, 'spaceAfter' => 150);
    $textRun = $section->addTextRun($paragraphStyle);
     $textRun->addText('Conforme al Artículo 23 del Acuerdo Superior 105 de 1993 o Estatuto General de la Universidad del Cauca, modificado por el Acuerdo Superior 025 de 2020 en su artículo segundo, corresponde al Rector autorizar las comisiones académicas al exterior no conducentes a título y no superiores a un año',  array('size' => 12));
    
        if (in_array($cargo_admin, ['JEFE', 'DECANO', 'DIRECTOR'])) {
$textRun->addText(', y presentar al Consejo Superior la solicitud de concesión de comisiones al exterior para empleados administrativos y docentes en ejercicio de cargos administrativos.',  array('size' => 12));}
    else
    {
...
$textRun->addText('Mediante la Resolución Rectoral 0901 de 2023, se reglamentaron los requisitos para la autorización y concesión de las comisiones académicas al interior y al exterior del país de los profesores, conforme al Proyecto Educativo del Programa – PEP, Proyecto Educativo Institucional – PEI y con el Plan de Desarrollo Institucional – PDI 2023-2027 ',  array('size' => 12));
...
$textRun->addText('De conformidad con los documentos remitidos por el presidente del Consejo de Facultad, que soportan la solicitud de la comisión, resulta pertinente para la Universidad la asistencia '. $enmodalidad.' ',  array('size' => 12));
...
// Añadir el título centrado
$section->addText('RESUELVE:', array('bold' => true, 'size' => 12), array('alignment' => Jc::CENTER,'spaceAfter' => 10)); 
$section->addTextBreak(1);
    ...
    $textRun->addText('ARTÍCULO PRIMERO: ', array('bold' => true,  'size' => 12));
     ...
     $textRun->addText('Conceder comisión académica al exterior,'. $enmodalidad. ' '.obtenerFechasFormateadas($fechainicio1, $fechafin1) . ', ',  array('size' => 12));
    
$textRun->addText($saludo. ' ',  array('size' => 12));
$textRun->addText($nombre_profesor, array('bold' => true,  'size' => 12));
$textRun->addText(', ',  array('size' => 12));
$textRun->addText($identificado . ' con la cédula de ciudadanía número ',  array('size' => 12));
$textRun->addText($documento_tercero, array('bold' => true,  'size' => 12));
...
$paragraphStyle = array('alignment' => Jc::BOTH, 'spaceAfter' => 150);
    ...
    $textRun->addText('PARÁGRAFO: ', array('bold' => true,  'size' => 12));
     ...
     $textRun->addText('La comisión autorizada '. $texto_erogaciones . '.',  array('size' => 12));    
    $paragraphStyle = array('alignment' => Jc::BOTH, 'spaceAfter' => 150);
    $textRun = $section->addTextRun($paragraphStyle);
...
if ($cargo_admin == "DECANO") {
    ...
    $textRun->addText('Como deber especial, ' . $comisionado . ' rendirá a la Decanatura de su Facultad con copia a la Vicerrectoría Académica, un informe escrito dentro de los diez (10) días siguientes al vencimiento de la comisión. En el informe se presentarán los aspectos pertinentes a los objetivos específicos de la unidad académica y generales de la Institución, de lo cual, se dejará constancia en el Acta de Reunión de Departamento.',  array('size' => 12));
}
    ...
    $textRun->addText('PARÁGRAFO: ', array('bold' => true,  'size' => 12));
     ...
     $textRun->addText('La Administración Universitaria se abstendrá de tramitar nueva comisión académica '. $saludo . 'de no satisfacer los requerimientos estipulados en la presente Resolución, por cuyo cumplimiento velará la Decanatura de su respectiva Facultad.',  array('size' => 12)); 
    ...
    $textRun->addText('ARTÍCULO TERCERO: ', array('bold' => true,  'size' => 12));
     ...
     $textoCopia = 'Enviar copia del presente acto administrativo a la Oficina de Relaciones Interinstitucionales e Internacionales (orii@unicauca.edu.co), '
    . $facultad_min . ' ('.$email_fac.'), División de Gestión del Talento Humano (rhumanos@unicauca.edu.co), '
    . 'División de Gestión Financiera (financiera@unicauca.edu.co), '
    . 'al Área de Seguridad y Salud en el Trabajo (saludocu@unicauca.edu.co), '
    . 'Vicerrectoría Académica (viceacad@unicauca.edu.co)';
// Validar si debe agregarse Vicerrectoría de Investigaciones
$esProyecto = stripos($cargo_a, 'PROYECTO') !== false;
$excepcionProyecto = (stripos($cargo_a, 'GERENCIA DE PROYECTOS') !== false ||
                      stripos($cargo_a, 'ORGANIZACIONES Y PROYECTOS') !== false);

if (stripos($cargo_a, 'VRI') !== false || 
    (!$excepcionProyecto && $esProyecto) || 
    stripos($cargo_a, 'VICERRECTORÍA DE INVESTIGACIONES') !== false || 
    stripos($cargo_a, 'VICERRECTORIA DE INVESTIGACIONES') !== false) {
        
    $textoCopia .= ', Vicerrectoría de Investigaciones (vri@unicauca.edu.co)';
}

// Validar si debe agregarse Centro de Regionalización
if (stripos($cargo_a, 'REGIONALIZACIÓN') !== false || 
    stripos($cargo_a, 'REGINALIZACION') !== false || 
    stripos($cargo_a, 'REGIONALIZACION') !== false) {
        
    $textoCopia .= ', Centro de Regionalización (regionalizacion@unicauca.edu.co)';
}

// NUEVO BLOQUE: Validar si debe agregarse Vicerrectoría de Cultura y Bienestar
if (stripos($cargo_a, 'CULTURA Y BIENESTAR') !== false || 
    stripos($cargo_a, 'VICECULTURA') !== false || 
    stripos($cargo_a, 'VICERRECTORÍA DE CULTURA Y BIENESTAR') !== false) {
        
    $textoCopia .= ', Vicerrectoría de Cultura y Bienestar (vicecb@unicauca.edu.co)';
}

// Siempre agregar el interesado y profesor
$textoCopia .= ' y '.$alinteresado.', '.$nombresOrdenadosp.' ('.$email_profesor.').';

// Insertar al documento Word
$textRun->addText($textoCopia, array('size' => 12));

  $paragraphStyle = array('alignment' => Jc::BOTH, 'spaceAfter' => 150);
    $textRun = $section->addTextRun($paragraphStyle);
      $textRun->addText('Se expide en Popayán (Cauca), ',  array('size' => 12));
...
// Añadir el título centrado
    ...
    $section->addText($rector_mayusculas, array('bold' => true, 'size' => 12), array('alignment' => Jc::CENTER, 'spaceAfter' => 0));   

    $section->addText($cargor, array('bold' => true, 'size' => 12), array('alignment' => Jc::CENTER, 'spaceAfter' => 0));  
...
$textRun = $section->addTextRun($paragraphStyle);
if ($tipo_vice <> "propiedad") {
    $nombrarvice = preg_replace('/^(La |El )/', '', $nombrarvice); // Elimina el primer artículo
    $nombrarvice = trim($nombrarvice); // Elimina espacios al inicio y al final
    $nombrarvice = ' (' . $nombrarvice . ')'; // Añade paréntesis
    $textRun->addText('Autorizó: ' . $autorizo, array('size' => 8, 'italic' => true));
    $textRun->addText($nombrarvice, array('size' => 8, 'italic' => true));
}
    else
    {
